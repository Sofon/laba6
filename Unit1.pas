...
begin
   flag:=True;
    if rg1.ItemIndex=0 then  //Ââîä èç ôàéëà
      begin
        if dlgOpen1.Execute then
         begin
            ...
            Reset(f);
              try
                ...
                if not ((n>0) and (n<=20)and(m>0)and(m<=20)) then
                  begin
                   flag:=False;
                   ShowMessage('Íåêîğåêòíûé ğàçìåğ ìàòğèöû');
                  end
                else
                 begin
                   SetLength(a,n,m);
               ...
               flag:=False;
               ShowMessage('Îøèáêà ïğè ââîäå/âûâîäå èñõîäíûõ äàííûõ');
              end;
            finally
             CloseFile(f);
             end;
           ...
           flag:=False;
           ShowMessage('Îøèáêà ïğè îòêğûòèè ôàéëà');
          end;
       end
    else
        flag:=False;
  end
 else //ââîä èç ñåòêè ñòğîê
  begin
  n:=ud1.Position;
  m:=ud2.Position;
  SetLength(a,n,m);
  try
   ...
   flag:=False;
   ShowMessage('Îøèáêà ïğè ââîäå èç ñåòêè ñòğîê ıëåìåíòà A['+ IntToStr(i+1)+','+IntToStr(j+1)+']');
   end;
 end;
   ...
   //Ğåøåíèå
        ...
        flag2:=PoiskOtr(n,m,ti,tj,iotr,jotr,a);
           if flag2 then
             begin
              ...
              Label3.Caption:=('Íîìåğ ıëåìåíòà ğàâåí ['+IntToStr(maxi+1)+','+IntToStr(maxj+1)+']');
             end
           else
           begin
           ShowMessage('Îòğèöàòåëüíûõ íåòó');
           Label3.Caption:=('Çàäà÷à åùå íå ğåøàëàñü');
          end;
        end
      else
      begin
    ...
    else Label3.Caption:=('Çàäà÷à åùå ğåøàëàñü');
end;
procedure TForm1.edt1Change(Sender: TObject);
 begin
  StringGrid1.RowCount := ud1.Position+1;// + îäèí ôèêñèğîâàííàÿ ñòğî÷êà
...
procedure TForm1.edt2Change(Sender: TObject);
 begin
  StringGrid1.ColCount:= ud2.Position+1; //   + îäèí ôèêñèğîâàííûé ñòîëáåö
...
try
  prov:=StrToInt(edt3.Text);
   ...
   ShowMessage('T ââåäåí íå ïğàâèëüíî');
   edt3.Text:='5';
end;
end;
